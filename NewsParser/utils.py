...
def remove_noise_node(element, noise_xpath_list):
    # print(etree.tostring(element, encoding='utf-8').decode())
    ...
    for noise_xpath in noise_xpath_list:
        ...
        if len(nodes)>0:
            # print("nodes",nodes[0])
            # print(etree.tostring(nodes[0], encoding='utf-8').decode())
            # print("noise_xpath", noise_xpath)
            pass
        for node in nodes:
            remove_node(node)
    return element
...
#def read_config():
 #   pwdpath = os.getcwd()
  #  pwdpath = pwdpath.replace("\\", "/")
   # print(pwdpath)
    #with open(pwdpath + '/NewsParser/gne.conf', encoding='utf-8') as f:
    #with open(pwdpath + '/../NewsParser/gne.conf', encoding='utf-8') as f:
     #   config_text = f.read()
    #if config_text:
